...
def fetch_and_update_product_info_from_openfoodfacts(price: Price):
    ...
    if existing_product_qs.count():
        ...
        print("Price task OFF info: found existing_product")
...
def fetch_and_update_location_info_from_openstreetmap(price: Price):
    ...
    existing_product_qs = (
        ...
        .exclude(location_osm_name__isnull=True)
        .exclude(product_osm_name__exact="")
    )
    if existing_product_qs.count():
        existing_product = existing_product_qs.first()
        print("Price task OSM info: found existing_product")
    ...
    try:
        ...
        if "display_name" in response:
            price.location_osm_name = response["display_name"]
        if "lat" in response:
            price.location_osm_lat = response["lat"]
        if "lon" in response:
            price.location_osm_lon = response["lon"]
        price.save()
    except Exception as e:
